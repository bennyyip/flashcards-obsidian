...
import type { Regex } from 'src/conf/regex'
...
export class Parser {
  ...
  private parseLine(str: string, vaultName: string) {
    return this.htmlConverter.makeHtml(
      this.mathToAnki(
        this.substituteObsidianLinks(this.substituteImageLinks(this.substituteAudioLinks(str)), vaultName),
      ),
    )
  }
  ...
  private substituteObsidianLinks(str: string, input_vaultName: string) {
    const linkRegex = /\[\[(.+?)(?:\|(.+?))?\]\]/gim
    const vaultName = encodeURIComponent(input_vaultName)
  ...
  private substituteImageLinks(input_str: string): string {
    let str = input_str
  ...
  private mathToAnki(input_str: string) {
    let str = input_str
    str = str.replace(this.regex.mathBlock, (match, p1, p2) => '\\\\[' + escapeMarkdown(p2) + ' \\\\]')

    str = str.replace(this.regex.mathInline, (match, p1, p2) => '\\\\(' + escapeMarkdown(p2) + '\\\\)')
  ...
  private getEmbedWrapContent(embedMap: Map<any, any>, input_embedContent: string): string {
    let embedContent = input_embedContent
